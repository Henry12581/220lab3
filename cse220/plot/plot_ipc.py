...
def get_IPC(descriptor_data, sim_path, output_dir):
    ...
    try:
        for config_key in descriptor_data["configurations"].keys():
            ...
            for benchmark in benchmarks_org:
                ...
                try:
                    with open(exp_path + config_key + '/memory.stat.0.csv') as f:
                        lines = f.readlines()
                        for line in lines:
                            if 'L1_HIT_pct' in line:
                                tokens = [x.strip() for x in line.split(',')]
                                IPC = float(tokens[1])
                                break
                except FileNotFoundError:
                    print(f"Warning: File for {benchmark} not found.")
                except ValueError:
                    print(f"Warning: Invalid data for {benchmark}.")
            ...
            if cnt_benchmarks > 0:
                ipc_config.append(avg_IPC_config / cnt_benchmarks)
            else:
                ipc_config.append(float('nan'))
            ipc[config_key] = ipc_config

        benchmarks.append('Avg')
        plot_data(benchmarks, ipc, 'IPC', output_dir + '/base_pref_off_l1hit_pct.png')

        output_to_txt(benchmarks, ipc, output_dir)
...
def plot_data(benchmarks, data, ylabel_name, fig_name, ylim=None):
    print(data)
    colors = ['#800000', '#911eb4', '#4363d8', '#f58231', '#3cb44b', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe',
              '#e6beff', '#e6194b', '#000075', '#800000', '#9a6324', '#808080', '#ffffff', '#000000']
    ...
    for key in data.keys():
        ...
        if idx % 2:
            ...
            hatch = '///'
        ax.bar(ind + (start_id + idx) * width, data[key], width=width, fill=False, hatch=hatch, color=colors[idx],
               edgecolor=colors[idx], label=key)
        idx += 1
...
def output_to_txt(benchmarks, ipc_data, output_dir):
    for config_key in ipc_data.keys():
        filename = os.path.join(output_dir, f'l1_hit_pct_{config_key}.txt')
        with open(filename, 'w') as txt_file:
            txt_file.write("Benchmark\tL1_HIT_pct\n")
            for i, benchmark in enumerate(benchmarks):
                if benchmark == 'Avg':
                    benchmark_name = 'Avg'
                else:
                    # Flatten the benchmark name list
                    benchmark_name = '/'.join(benchmark)
                L1_HIT_pct = ipc_data[config_key][i]
                txt_file.write(f"{benchmark_name}\t{L1_HIT_pct}\n")
        print(f"L1_HIT_pct data written to {filename}")
